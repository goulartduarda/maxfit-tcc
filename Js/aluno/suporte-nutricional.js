// ===== Suporte Nutricional (versão simplificada sem API Key) =====
...
function onBuscar() {
  ...
  if (!local) {
    alert("Digite uma cidade ou bairro para buscar nutricionistas.");
    return;
  }

  // Cria busca simples no Google Maps
  const query = encodeURIComponent(`Nutricionista em ${local}`);
